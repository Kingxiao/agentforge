...
Image inputs are token sinks — must declare in Spec phase:
...
### Large Text Content Truncation Strategy (Webhook / Tool-output Agent — Required Reading)
...
**Selection routing**: Multi-tenant + complex context routing → prefer PubSub Event Loop (OpenCode style) or Plugin Gateway (OpenClaw style) — these two paradigms have more built-in support for multi-tenant isolation. → See `/agentforge-architecture` for architectural paradigms
...
Common wrong layout (avoid):
  [System prompt]
  [Conversation history]
  [All retrieved fragments #1-#5]  ← All piled in middle → empirically, Faithfulness drops 15-20%
  [Current user question]
```
...
Before finalizing model selection, must WebFetch the following real-time data (don't use static cached knowledge from training data):
...
Prompt variants (Decision 11) handle *structural* differences — XML vs. Markdown, tag placement, section ordering. A different problem: the same instruction produces different *behavioral* outputs across model families. GPT/Codex tends to ask clarifying questions instead of acting; Gemini skips parallel tool calls; newer reasoning models need explicit scratchpad boundaries.

**Hermes approach [HR]**: Separate constants per model family, generated by automated behavioral benchmarks, committed to source:

```python
OPENAI_MODEL_EXECUTION_GUIDANCE = """
<tool_persistence>
Continue using tools until the task is fully complete. Do not ask the user
for confirmation between tool calls unless you are blocked by missing information.
</tool_persistence>
<mandatory_tool_use>
Always use tools to accomplish tasks. Never describe what you would do — do it.
</mandatory_tool_use>
<act_dont_ask>
Act on reasonable assumptions. State your assumption, perform the action,
report the result. Do not ask "should I proceed?"
</act_dont_ask>
"""

GOOGLE_MODEL_OPERATIONAL_GUIDANCE = """
- Use absolute paths for all file operations
- Batch independent tool calls in parallel (use multiple tool_calls in one response)
- Do not re-read files you have already read in this session unless the content may have changed
"""
```

These are injected at the API call layer, not in the base system prompt — they are applied on top of prompt variants, not as replacements.

### How Behavioral Guidance Gets Generated

The key architectural decision: **don't author these constants by hand**. Each constant is a distillation of automated benchmark results:

```
1. Define behavioral benchmark suite
   (e.g., "task requiring 5 sequential tool calls" × 20 variants × 3 repetitions)
2. Run against each target model family
3. Identify systematic failure modes
   (GPT: stops and asks at step 3; Gemini: uses sequential where parallel would work)
4. Write minimal instruction that eliminates the failure mode
5. Verify: re-run benchmark, confirm improvement
6. Commit constants to source with verification date
```

The constants become stale when models update. Verification date in code makes this explicit:

```python
# OPENAI_MODEL_EXECUTION_GUIDANCE — verified: 2026-03-15 against gpt-4.1, codex-mini
# Re-verify after any OpenAI model release that changes tool_call behavior
```
...
|-----------|-----------------|------|
| Single-model, fixed provider | No — hardcode to that model's known needs | — |
| Multi-provider from day one | Yes | ~1 day per model family for benchmark + constant |
| Agent OS / gateway (serves N model backends) | Essential — users bring their own models | Build as first-class routing layer |
| Research agent targeting best-available model | Yes, but date-stamp everything | Benchmark re-run on each model release |

**Integration with Decision 11 (Prompt Variants)**:

```
System prompt assembly order:
1. Base system prompt (invariant content)
2. PromptVariant sections (structural adaptation per model family)
3. Per-model behavioral guidance (execution correction, injected last)
```

Behavioral guidance injects last so it can override any structural section's default behavioral assumptions.
...
1. **1M token context windows becoming mainstream** — Claude and Gemini both support 1M+ token contexts, but "fitting in" doesn't equal "using well." Empirical testing shows significant model performance degradation after 200K tokens; core context engineering is still "less but better" not "more is better."
